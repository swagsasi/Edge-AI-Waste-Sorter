...
import org.tensorflow.lite.examples.imageclassification.BleManager
...
class CameraFragment : Fragment(), ImageClassifierHelper.ClassifierListener {
    ...
    override fun onViewCreated(view: View, savedInstanceState: Bundle?) {
        ...
        BleManager.startScanAndConnect(requireContext())
    }
    ...
    override fun onResults(
        ...
        // Bluetooth logic added before updating the UI
        results?.firstOrNull()?.categories?.firstOrNull()?.let { category ->
            val label = category.label
            val score = category.score

            // If it's Organic and confidence is greater than 90%
            if (label == "Organic" && score >= 0.90f) {
                BleManager.sendCommand("G")
            } else {
